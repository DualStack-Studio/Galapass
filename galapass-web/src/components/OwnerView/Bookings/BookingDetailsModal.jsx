...
const BookingDetailsModal = ({ isOpen, booking, onClose }) => {
    ...
    const formatDate = (date) => {
        ...
        const monthNames = t('calendar.monthNames', { returnObjects: true, lng: lang });
        const dayNames = t('calendar.fullDayNames', { returnObjects: true, lng: lang });
            ...
            <div className={`bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto ${isOpen ? 'animate-slide-up' : 'animate-slide-down'}`}>
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold text-gray-900">{t('booking_management.booking_details')}</h2>
                        <button
                            onClick={onClose}
                    ...
                    <div className="space-y-6">
                        {/* Tour Information */}
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('booking_management.tour_information')}</h3>
                            <div className="flex items-start space-x-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-gray-600 mt-1">
                                        {t('booking_management.price_per_person')}: ${tour.price}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Booking Details */}
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('booking_management.booking_details')}</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-gray-500">{t('booking_management.tour_date')}</p>
                                    <p className="font-medium text-gray-900">
                                        {formatDate(new Date(booking.date))}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">{t('booking_management.number_of_guests')}</p>
                                    <p className="font-medium text-gray-900">{booking.numberOfPeople}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">{t('booking_management.total_amount')}</p>
                                    <p className="font-medium text-gray-900">${booking.totalPaid}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">{t('booking_management.status')}</p>
                        ...
                        {booking.guides && booking.guides.length > 0 && (
                            <div>
                                <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('booking_management.assigned_guides')}</h3>
                                        ...
                                        <div key={guide.id} className="flex items-center space-x-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center text-sm text-gray-500 mt-1">
                                                    ...
                                                    {guide.email || t('booking_management.no_email')}
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        {/* Guest Information */}
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('booking_management.guest_information')}</h3>
                            <div className="space-y-3">
                                {booking.tourists.map((tourist) => (
                                    <div key={tourist.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                                        ...
                                        >
                                            {tourist.profilePhoto ? (
                                                <img
                                                    ...
                                                    alt={tourist.name || t('booking_management.unknown_guest')}
                                        ...
                                        <div>
                                            <p className="font-medium text-gray-900">{tourist.name || t('booking_management.unknown_guest')}</p>
                                            <p className="text-sm text-gray-500">{tourist.email || t('booking_management.no_email')}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="flex space-x-3">
                        {booking.status === 'PENDING' && (
                            <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
                                {t('booking_management.confirm_booking')}
                            </button>
                        )}
                        {booking.status !== 'CANCELED' && (
                            <button className="border border-red-300 text-red-700 hover:bg-red-50 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer">
                                {t('booking_management.cancel_booking')}
                            </button>
                        )}
                    </div>
                </div>
