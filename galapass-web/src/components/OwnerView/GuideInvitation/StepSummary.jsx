...
const StepSummary = ({ guide, company }) => {
    ...
    return (
        <div className="text-center py-16">
            ...
            <h2 className="text-3xl font-bold mb-4">{t('guide_invitation.guideInvitedSuccessTitle')}</h2>
            <p className="text-lg text-gray-600 mb-8">
                {guide.name} {t('guide_invitation.successfullyInvitedTo')} <strong>{company.name}</strong>.
            </p>

            <div className="bg-blue-50 rounded-xl p-6 max-w-md mx-auto mb-8">
                <div className="flex items-center justify-center mb-4">
                    <Users className="h-5 w-5 text-blue-600 mr-2" />
                    <span className="text-blue-800 font-medium">{t('guide_invitation.teamUpdated')}</span>
                </div>
            ...
            >
                ...
                {t('guide_invitation.backToDashboard')}
            </button>
        </div>
    );
};
